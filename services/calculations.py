...
def get_dashboard_data(user_id: int):
    """Return the figures needed to display the dashboard for today for a specific user."""
    today = date.today()
    ...
    with get_connection() as connection:
        ...
        upcoming_recurring = connection.execute(
            ...
            GROUP BY category
            """,
            (user_id, last_day_of_month.isoformat()),
        ).fetchall()
        goals = connection.execute(
            "SELECT target_amount, saved_amount, deadline FROM savings_goals WHERE user_id = ? AND deadline IS NOT NULL",
            (user_id,),
        ).fetchall()
...
def get_analytics_data(user_id: int):
    """Return this month's spending figures and simple rule-based insights for a user."""
    today = date.today()
    current_month = today.strftime("%Y-%m")
    ...
    with get_connection() as connection:
        ...
        monthly_trend_rows = connection.execute(
            ...
            ORDER BY month
            """,
            (user_id, (today - timedelta(days=183)).isoformat()),
        ).fetchall()
